...
class ThreeBodySimulation:
    ...
    def step(self):
        ...
        try:
            ...
            if not np.all(np.isfinite(self.positions)) or not np.all(
                    np.isfinite(self.velocities)):
                print(
                    "Invalid positions or velocities detected, resetting simulation"
                )
                self.reset_to_default()
                return
            ...
            sol = solve_ivp(self.three_body_ode,
                            t_span,
                            y0,
            ...
            if sol.success and len(sol.y) > 0:
                ...
                if np.all(np.isfinite(new_positions)) and np.all(
                        np.isfinite(new_velocities)):
                    ...
                    print(
                        "Numerical instability detected, using fallback method"
                    )
                    self.simple_step()
            else:
                print(
                    f"ODE solver failed: {sol.message if hasattr(sol, 'message') else 'Unknown error'}"
                )
                self.simple_step()
        ...
        for i in range(3):
            for j in range(3):
                if i != j:
                    ...
                    if r_mag > 1e-8:
                        force_mag = self.G * self.masses[i] * self.masses[
                            j] / (r_mag**2)
